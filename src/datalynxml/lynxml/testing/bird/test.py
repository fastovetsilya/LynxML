...
if __name__ == "__main__":
    ...
    # Load json file
    bird_dev_json_path = "/home/ilya/Desktop/BIRD/dev/dev/dev.json"
    with open(bird_dev_json_path, "rt") as f:
        bird_dev_list = json.loads(f.read())

    # Print examples
    logger.debug(f"Loaded {len(bird_dev_list)} examples from {bird_dev_json_path}")
    logger.debug(f"First example is: {bird_dev_list[0]}")

    # Select random examples to test
    test_examples_ids = np.random.choice(len(bird_dev_list), size=100, replace=False)
